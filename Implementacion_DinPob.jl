...
# ╔═╡ d9fba350-e126-4475-a77e-e4b0d3b89ecd
begin
	using ForwardDiff
	using LinearAlgebra
	using Plots
...
# ╔═╡ 9ae57b44-371d-445f-ae21-0ec3f7149865
md"""
Juan Diego Murcia Porras, 2025
"""

# ╔═╡ b17fbf67-9a5a-4481-9afe-2267fb531f6b
md"""
# Implementación del método iLQR con restricciones en el control a la explotación de una especie
"""

# ╔═╡ 0995e589-bd69-4806-b071-7f07a7edfd42
md"""
La implementación aquí realizada se inspira en el método iLQR presentado en [2] y [3], para el que se aplican modificaciones para considerar restricciones tipo caja en el control siguiendo lo expuesto en [1].
"""

# ╔═╡ bc382a67-d58d-404d-a731-8154721cfc40
md"""
Problema a resolver:

$\min_{u(t)} -\int_0^{t_f}e^{-\delta t}(pqx-cq)u(t)~dt$
sujeto a la dinámica

$\begin{cases}\dot{x}(t)=rx(1-\frac{x}{K})-qux\\
x(0)=x_0\end{cases}$
"""

# ╔═╡ 0477a8c9-6f48-4056-ab3e-1429a81cbfb2
begin
	global K,r,delta,q,p,c,Emin,Emax
	K=10
	r=0.15
	delta=0.2
	q=0.7
...
# ╔═╡ 233e1160-e768-11ef-3ee4-e92f666d21a3
begin
	global dimx,dimu
	dimx=1
	dimu=1
	function f(x,u)
		return r*x .*([1]-(1/K)*x)-q*u .*x
	end
end

# ╔═╡ 932ead00-7684-4098-8ea9-30c8a9175e8d
begin
	global h, t_final, Nt, t_div
	h=0.025;
	t_final=30;
...
# ╔═╡ 9a02d7ab-4054-4e85-8a5a-1a5adab29343
...
# ╔═╡ f287d762-fd2c-4034-b381-1521c6a41e01
...
# ╔═╡ 67d7c0c8-db0c-4f7e-80e2-e0a86557187f
#### Declaración del costo de etapas
function stage_cost(x,u,m)
	return -(exp(-delta*t_div[m])*(p*q*u.*x-c*u))[1]
end

# ╔═╡ 025d7486-24c4-4acf-89ca-51db218b00d6
function total_cost(x_traj,u_traj,init=1)
	L=0
	for k in init:Nt-1
		L = L+stage_cost(x_traj[:,k],u_traj[:,k],k)[1]
	end
	return L
end

# ╔═╡ d45aa96d-145a-49c6-8749-89d3d64e0312
function Comm(m, n)
    K = zeros(Int, m * n, m * n)
    for i in 1:m
        for j in 1:n
...
# ╔═╡ cd0b0133-9901-4a2b-86cc-9e002b2f4804
begin
	###minimizar 1/2 x^T H x+q^T x sujeta a b_i\lea x\leq b_f
	###Retorna los índices correspondientes a 
	function clamped_newton_control_qp(H,q,b_i,b_s, x_0, dim_x)
		xn=copy(x_0)
		for i in 1:dim_x
			if xn[i]<b_i[i]
				xn[i]=b_i[i]
		...
		while true
			...
			fr=[]
			it+=1
			deltax=zeros(dim_x, 1)
			g=q+H*xn
			for i in 1:dim_x
				if (xn[i]==b_i[i] && g[i]>0) || (xn[i]==b_s[i] && g[i]<0)
					c=push!(c,i)
				else
					fr=push!(fr,i)
				end
			...
			H_ff=H[fr,fr]
			H_fc=H[fr,c]
			g_f=q[fr]+H_ff*xn[fr]+H_fc*xn[c]
			if isempty(fr)
				global H_ff
				H_ff=[0]
				break
			end
			if maximum(abs.(g_f[:]))<1.e-12
				break
			end
			deltax[fr]=-H_ff\g_f
			alpha=1
			x_nuevo=xn+alpha*deltax
			for i in 1:dim_x
				if x_nuevo[i]<b_i[i]
					x_nuevo[i]=b_i[i]
				end
			...
			function obj(t)
				return (0.5* t'*H*t+q'*t)[1]
			end
			while (obj(xn)-obj(x_nuevo))/(dot(g,xn-x_nuevo))<0.1
				alpha=0.5*alpha
				x_nuevo=xn+alpha*deltax
				for i in 1:dimx
					if x_nuevo[i]<b_i[i]
...
# ╔═╡ f3e7499d-fbc1-487c-b2c2-4920b62b644b
begin
	function backward_pass(x_traj,u_traj)
		d_loc=ones(dimu,Nt-1)
		...
		for k in Nt-1:-1:1
			l_x=-exp(-delta*t_div[k])*p*q*u_traj[:,k] ##lx(z)=stage_cost(z,u[:,k])\\l_x=ForwardDiff.jacobian(lx,xtrahj[:,k])
			l_xx=[0]
			##l_xx=ForwardDiff.hessian(lx,xtraj[:,k])
			l_xu=-[exp(-delta*t_div[k])*p*q]
			##En caso de tener otra relación,implementar forwardDiff para hallar esta derivada.
			l_u=-exp(-delta*t_div[k])*p*q*x_traj[:,k]+[c*q] #####lu(w)=stage_cost(x[:,k],w)\\l_u=ForwardDiff.jacobian(lx,utraj[:,k])
			l_uu=[0]
			...
			#print("G_uu:",G_uu,"\n\n")
			beta=0.0001
			if minimum(diag(G_uu))>0
				tau=0
			else 
			tau=-minimum(diag(G_uu))+beta
			end
			while !isposdef(G_uu)
				G_uu=G_uu+tau*I(dimu)
				tau=max(2*tau,beta)
			end
			if k==Nt-1
				d_in=0.5*(Emin+Emax)
			...
			d_loc[:,k],free,Guu_free=clamped_newton_control_qp(G_uu,g_u,Emin-u_traj[:,k],Emax-u_traj[:,k], d_in, dimu)
			d_loc[:,k]=-d_loc[:,k]
			if free!=[]
				K_loc[free,:,k]=Guu_free\G_ux[free, :]
			end
...
# ╔═╡ a574bf78-6e39-4bc5-91cd-be5580419ce5
begin
####Definimos parametros iniciales
	...
	xtraj[:,1]=x0
	u0=1.3
	utraj=u0*ones(dimu, Nt-1)
	for k in 2:Nt
		xtraj[:,k]=dynamics_rk4(xtraj[:,k-1],utraj[:,k-1])
	end
end

# ╔═╡ 9551465b-bdd8-4a63-86d7-6452292e0097
begin
	global Nt,dimx,dimu
	function DDP_iLQR(xtraj, utraj, d=ones(Nt-1),K=zeros(dimu,dimx,Nt),P=zeros(dimx,dimx,Nt), p=zeros(dimx,Nt))
		...
		m=0
		delta_costo=10
		while maximum(abs.(d[:]))>1.e-3 && delta_costo>1.e-10
			m+=1
			iter+=1
			...
			for k in 1:(Nt-1)
				val_u=utraj[:,k]-alpha.*d[:,k]-K[:,:,k]*(xn[:,k]-xtraj[:,k])
				for j in 1:dimu
					if val_u[j]<Emin[j]
						un[j,k]=Emin[j]
					elseif val_u[j]>Emax[j]
						un[j,k]=Emax[j]
					else
						un[j,k]=val_u[j]
					end
			...
			while isnan(Jn[1]) || Jn>(J-1e-10*alpha*delta_J)
				print("Inicia búsqueda lineal\n")
				alpha=0.2*alpha
				### Se realiza la dinámica con el nuevo valor del control 
				for k in 1:(Nt-1)
					val_u=utraj[:,k]-alpha.*d[:,k]-K[:,:,k]*(xn[:,k]-xtraj[:,k])
				...
				Jn=total_cost(xn,un)
				print("Costo después de búsqueda lineal:",Jn)
			end
...
# ╔═╡ b6c94698-d31d-4b7c-b599-c6397f5e564b
xopt,uopt, dopt, Jopt, popt,Popt= DDP_iLQR(xtraj, utraj)

# ╔═╡ 43a9710e-e011-4841-9a15-59bf69f308a6
m=(K/4) * ((c/(p*q*K) + 1 - (delta/r)) + sqrt((c/(p*q*K) + 1 - (delta/r))^2 + (8*c*delta)/(p*q*K*r)))

# ╔═╡ 6c5c8ff2-e330-4d25-b662-7de336d63ca5
begin
	Plots.plot(t_div,xopt[1,:],title="Estado obtenido numéricamente \n para delta=$delta, u_0=$u0", label="Solución obtenida" ,lw=2, color="blue", xlabel="t", ylabel="x(t)", ylim=(0,x0[1]))
	Plots.plot!(t_div,m*ones(length(t_div)),label="Equilibrio singular" ,lw=1, color="blue", linestyle=:dash)
end

# ╔═╡ a88f5461-1dff-4a4f-b740-897c65f13b21
begin
	Plots.plot(t_div[1:Nt-1],uopt[1,:],title="Control óptimo obtenido numéricamente\n para delta=$delta, u_0=$u0", label="Solución obtenida" ,xlabel="t", ylabel="u(t)",lw=2, color=:purple, ylim=(0,Emax[1]))
end

# ╔═╡ 9eaf89b5-023f-470a-a01a-d6a1f2269107
Jopt

# ╔═╡ c5006770-25fb-4bb5-b268-4d3e7f9034d6
begin
	E_opt=r/q*(1-m/K)
end

# ╔═╡ e64e822e-c81a-472d-8c03-2979920c0aaf
begin
	if x0[1]<m
		for p in 1:2*Nt//3
			global xtrajeq, utrajeq, xopteq, uopteq, val
		####Definimos parametros iniciales
			xtrajeq=zeros(dimx, Nt)
			xtrajeq[:,1]=x0
			utrajeq=Emin*ones(dimu, Nt-1)
			for k in 2:Nt
				xtrajeq[:,k]=dynamics_rk4(xtrajeq[:,k-1],utrajeq[:,k-1])
				if xtrajeq[1,k]>m && k<Nt-p
					utrajeq[:,k]=[r/q*(1-xtrajeq[1,k]/K)]
				end
				if Nt-p<=k<Nt
					utrajeq[:,k]=Emax
				end
			...
			if p==1
				val=total_cost(xtrajeq,utrajeq)
				xopteq=xtrajeq
				uopteq=utrajeq
			end
			if total_cost(xtrajeq,utrajeq)<val
				val=total_cost(xtrajeq,utrajeq)
				xopteq=xtrajeq
				uopteq=utrajeq
			end
...
# ╔═╡ 81efcc19-0c96-48f1-85f5-2d6cf949032e
xopteq

# ╔═╡ 689338d1-f0ae-4a71-94d6-a1ad70ffea50
begin
	Plots.plot(t_div,xopteq[1,:],title="Estado óptimo basado en la teoría\npara delta=0.01", label="Solución obtenida" ,lw=2, color="blue", xlabel="t", ylabel="x(t)", ylim=(0,x0[1]))
	Plots.plot!(t_div,m*ones(length(t_div)),label="Equilibrio singular" ,lw=1, color="blue", linestyle=:dash)
end

# ╔═╡ dde9653c-8784-4e3c-a955-cfc3e126e882
begin
	Plots.plot(t_div[1:Nt-1],uopteq[1,:],title="Control óptimo basado en la teoría\n para delta=0.01", label="Solución obtenida" ,xlabel="t", ylabel="u(t)", lw=2, color=:purple, ylim=(0,Emax[1]))
end

# ╔═╡ 201b9818-e262-498d-aec9-5df946a2cce6
total_cost(xopteq,uopteq)

# ╔═╡ 22e83bca-4844-49f5-94d4-154512d77275
begin
    x_vf = range(0, 6, length=50) 
    function V(x, k)
        Pk = Popt[:, :, k]   
        pk = popt[:, k]      
...
# ╔═╡ 2b277904-17bd-49e5-8b60-26159065df32
begin
	plt=Plots.surface(t_div,x_vf, Z', xlabel="t", ylabel="x", zlabel="V(x,t)", title="V(t,x) alrededor de la trayectoria óptima",legend = :topright)
	V_cero = [V(xopt[1,k], k) for k in 1:Nt] 
	Plots.plot!(plt,  t_div, xopt[1,:], V_cero, label="V(t,x) sobre la trayectoria óptima",linewidth=1, color=:red)
end

# ╔═╡ 77c20b63-172a-495b-aadc-43b97a17d63e
begin
	fig = WGLMakie.Figure()
	...
	WGLMakie.lines!(ax, t_div, fill(m, length(t_div)), V_cero, 
                linewidth=2, color=:blue, label="Valor sobre punto singular")
...
# ╔═╡ 35e6891d-8f63-4d4d-b62b-1587fdf8b588
md"""
# Referencias
"""

# ╔═╡ 956365f6-5be2-4954-b4dd-1072ab5bb498
md"""
[1] Yuval Tassa, Nicolas Mansard, and Emo Todorov. Control-limited differential dynamic pro-
gramming. In Proceedings of the IEEE International Conference on Robotics and Auto-
mation (ICRA), pages 1168–1175. IEEE, 2014. URL https://homes.cs.washington.edu/
~todorov/papers/TassaICRA14.pdf.
...
manifest_format = "2.0"
project_hash = "b8a597c4c74525aae3bb399eb77a564c0324f0e3"
